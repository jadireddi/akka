...
* Management of Cluster membership is stalled during network partitions as described in a 
  separate section below. This means that nodes can't be added and removed during network
  partitions across data centers.  
...
* Downing/removal of nodes in the case of network partitions should typically be treated
  ...
  data center you want more aggressive downing for quick fail over.
...
messaging because it was separated into several Akka Clusters.
...
by setting the `akka.cluster.data-center` configuration property. A node can only belong 
to one data center and if nothing is specified a node will belong to the `default` data center.
...
TODO: code example showing cluster.selfDataCenter, member.dataCenter, cluster.allDataCenters
...
* `akka.cluster.multi-data-center.cross-failure-detector` for failure detection across different data centers
...
TODO: code example of proxies for own and other data centers
...
TODO: code example of proxies for other data centers
